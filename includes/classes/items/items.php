...
class items
{
  ...
  public static function add_access_query($current_entity_id,$listing_sql_query, $force_access_query = false)
  {
    ...
    {                    
      $users_fields = array(); 
      $fields_query = db_query("select f.id from app_fields f where f.type in ('fieldtype_users','fieldtype_user_roles','fieldtype_users_approve') and  f.entities_id='" . db_input($current_entity_id) . "'");
  ...
  public static function get_send_to($entity_id, $item_id, $item=false)
  {
  	...
  	$fields_query = db_query("select f.* from app_fields f where f.type in ('fieldtype_users_approve','fieldtype_user_roles','fieldtype_grouped_users','fieldtype_users') and  f.entities_id='" . db_input($entity_id) . "' ");
  	while($field = db_fetch_array($fields_query))
  	{
  		$cfg = new fields_types_cfg($field['configuration']);
