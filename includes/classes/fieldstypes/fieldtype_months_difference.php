...
class fieldtype_months_difference
{
	...
	static function prepare_procedure()
	{
		...
		$sql = "
CREATE FUNCTION `spruton_months_diff`(`start_date` INT, `end_date` INT, `inc_days` TINYINT(1)) RETURNS int(11)
BEGIN  				
...
END;";
		...
		{
			if($check['Name']=='spruton_months_diff')
			{
				$is_function = true;
			}
		}
	...
	public static function prepare_query($fields,$prefix = 'e', $single_select = false, $force_query = false)
	{
		...
		if($single_select)
		{
			$mysql_query =  "(spruton_months_diff(" . $start_date_field . "," . $end_date_field . "," . $cfg->get('calclulate_diff_days',0). "))";
		}
		else
		{
			$mysql_query =  "spruton_months_diff(" . $start_date_field . "," . $end_date_field . "," . $cfg->get('calclulate_diff_days',0). ") as field_" . $fields['id'];
		}
