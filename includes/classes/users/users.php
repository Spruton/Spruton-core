...
class users
{
  ...
  static public function has_access_to_assigned_item($entities_id, $items_id)
  {
    ...
    {	       	    	
	    $users_fields = array(); 
	    $fields_query = db_query("select f.id from app_fields f where f.type in ('fieldtype_users','fieldtype_user_roles','fieldtype_users_approve') and  f.entities_id='" . db_input($entities_id) . "'");
	    while($fields = db_fetch_array($fields_query))
	    {    
